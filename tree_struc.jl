...
mutable struct Node
    ...
    function Node(height::Float64, parent::Union{Int, Nothing}, level::Int, inter::Vector{Float64})
...
"""
function Tree(get_probs::Function, splits, x)
    ...
    #median
    add_node!(tree, get_probs([0, splits[2]], 1, x), nothing, [0, splits[2]]) #prob 1 to be in the root
    add_node!(tree, get_probs([splits[2], 1], 1, x), nothing, [splits[2], 1])

    #first half
    add_node!(tree, get_probs([0, splits[1]], 2, x), 1, [0, splits[1]])
    add_node!(tree, get_probs([splits[1], splits[2]], 2, x), 1, [splits[1], splits[2]])

    #second half
    add_node!(tree, get_probs([splits[2], splits[3]], 2, x), 2, [splits[2], splits[3]])
    add_node!(tree, get_probs([splits[3], 1], 2, x), 2, [splits[3], 1])
...
function add_node!(tree::Tree, height::Float64, i::Union{Int, Nothing}, rinter::Vector{Float64})
   isnothing(i) || i ≤ length(tree.nodes) || throw(BoundsError(tree, i)) # this throws the exception when the condition is met
...
function get_parent(tree::Tree, id::Int)::Node
    tree.nodes[tree.nodes[id].parent]
end
...
function split_node!(tree::Tree, id::Int, y::Float64)
    ...
    add_node!(tree, padre.height * y, id, pint0)
